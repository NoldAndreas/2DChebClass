function [V,VDiff,VInt] = Vext14(y1,y2,intBound)
    ...
    z           = (y2-pi/2).*y1*c2;
    V           = tanh(z);      
    dVdy1       = (1-(tanh(z)).^2).*(y2-pi/2)*c2;
    dVdy2       = (1-(tanh(z)).^2).*y1*c2;
    dVddy1      = -2*tanh(z).*(1-(tanh(z).^2)).*(y2-pi/2).^2*c2^2;
    dVddy2      = -2*tanh(z).*(1-(tanh(z)).^2).*(y1.^2)*c2^2;
    dVdy1dy2    = -2*tanh(z).*(1-(tanh(z)).^2).*y1.*(y2-pi/2)*c2^2 + ...
                  + (1-(tanh(z)).^2)*c2;
