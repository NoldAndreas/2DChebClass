function [D,sumDivD] = wallRP2D(r,sigmaH)
...
deltaDOzx = Rz.*( z2.*R3 + 3*z1z2.*Rz.*R5 );
...
deltaDRPzx = - Rx.*Rz.*( 2*R5 - 5*Rz.^2.*R7 );
...
% identity part of HI)
% zInv = 1./z;
% selfTermPar = - 9/32*diag(sigmaH)*zInv + 1/64*(diag(sigmaH)*zInv).^3
...
sumDivx = sum(divOx + divRPx,2);
sumDivz = sum(divOz + divRPz,2);

sumDivD = [sumDivx'; sumDivz'];
sumDivD = sumDivD(:);
