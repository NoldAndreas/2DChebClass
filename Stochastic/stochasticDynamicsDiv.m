function [x,p]=stochasticDynamicsDiv(f,x0,p0,optsPhys,optsStoc,plotPosMask)
...
switch type % Langevin or EM dynamics
    ...
    for t=2:tSteps+1
        ...
        if(HI)
            ...
            [D,divD]=makeHI(x(:,t-1),optsPhys);
           
            %DTemp = RP(x(:,t-1),optsPhys);
        ...
        x(:,t) = x(:,t-1) + divD*dt - dt*D*DV/kBT + sqrt(2) *sqrt(dt)*A*f(:,t-1);
