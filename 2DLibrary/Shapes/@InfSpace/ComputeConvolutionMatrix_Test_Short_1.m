function ComputeConvolutionMatrix_Test_Short_1(this)
...
%    M_conv_V = ComputeConvolutionMatrix(this,@g1g2,[],true);
%    M_conv_S = ComputeConvolutionMatrix(this,@g3,[],true);

    M_conv_New_M = ComputeConvolutionMatrix(this,@gMat,shapeParams);
%    M_conv_New_V = ComputeConvolutionMatrix(this,@g1g2,shapeParams);          
%    M_conv_New_S = ComputeConvolutionMatrix(this,@g3,shapeParams);

    N1 = this.Pts.N1;
    N2 = this.Pts.N2;

    conv_M = zeros(N1*N2,2,2);
%    conv_V = zeros(N1*N2,2);

    conv_New_M = zeros(N1*N2,2,2);
%    conv_New_V = zeros(N1*N2,2);

%    conv_New_S = M_conv_New_S*g4(X,Y);
%    conv_S     = M_conv_S*g4(X,Y);

    for k1 = 1:2
%        conv_New_V(:,k1) = M_conv_New_V(:,:,k1)*g4(X,Y);
%        conv_V(:,k1) = M_conv_V(:,:,k1)*g4(X,Y);
        for k2 = 1:2 
...
%     disp(['InfSpace: ComputeConvolutionMatrix_Test Vector ''pointwise'' error: ' ...
%             num2str( sum(abs(exact14-conv_New_V(:,1)).^2) / sum(abs(exact14).^2) )])
% 
%     disp(['InfSpace: ComputeConvolutionMatrix_Test Vector ''pointwise'' error: ' ...
%             num2str( sum(abs(exact24-conv_New_V(:,2)).^2) / sum(abs(exact24).^2) )])
...
%     disp(['InfSpace: ComputeConvolutionMatrix_Test Scalar ''standard'' error: ' ...
%             num2str( sum(abs(exact34-conv_S).^2) / sum(abs(exact34).^2) )])


%     disp(['InfSpace: ComputeConvolutionMatrix_Test Vector ''standard'' error: ' ...
%             num2str( sum(abs(exact14-conv_V(:,1)).^2) / sum(abs(exact14).^2) )])
% 
%     disp(['InfSpace: ComputeConvolutionMatrix_Test Vector ''standard'' error: ' ...
%             num2str( sum(abs(exact24-conv_V(:,2)).^2) / sum(abs(exact24).^2) )])
