...
%plotPos = [1;ceil(nSaves/4);ceil(nSaves/2)];
plotPos = [1;ceil(nSaves/3);ceil(2*nSaves/3)];
...
for iPlot=1:nPlots  
    ...
    %    outputFile=optsPlot.IFFiles{iPlot};
       ...
       %----------------------------------------------------------------------
        ...
        plotRhoVdistStocColour2D(rhoError,[],boxes,optsPlot,handlesS);
        hold(hSa,'on');
        hold(hPSa,'on');
