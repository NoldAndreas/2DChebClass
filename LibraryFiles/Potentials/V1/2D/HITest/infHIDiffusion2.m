function [VBack_S,VAdd_S]=infHIDiffusion2(y1S,y2S,t,optsPhys)
...
%     alpha2 = 1;
    ...
    VAdd        = -V0add.*exp((-(y1S-y1t).^2/sigma1Add - (y2S-y2t).^2/sigma2Add));
    VAdd(abs(y1S)==inf |  abs(y2S)==inf) = 0;
    
    DVAddDy1   = -2*(y1S-y1t).*VAdd/sigma1Add;
    DVAddDy2   = -2*(y2S-y2t).*VAdd/sigma2Add;
