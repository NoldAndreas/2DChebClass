function optsStruct=setUParams(optsStruct)
...
vars=optsStruct.flowParamsNames;
...
if(optsStruct.anyStoc)
    ...
    %----------------------------------------------------------------------
    % Set up stochastic U parameters in potParams
    %----------------------------------------------------------------------
    ...
    flowParamsCmd=['optsStruct.flowParams = struct(' addVarText ');'];
    eval(flowParamsCmd)
...
if(optsStruct.anyDDFT)
    ...
    for iDDFT=1:nDDFT
        ...
        flowParamsCmd=['optsStruct.flowParamsDDFT{iDDFT} = struct(' addVarText ');'];
