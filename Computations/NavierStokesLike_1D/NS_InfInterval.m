function [data,optsPhys,optsNum,optsPlot] = NS_InfInterval(optsPhys,optsNum,optsPlot)

    if(nargin == 0)
        [data,optsPhys,optsNum,optsPlot] = Test_NS_InfInterval();
        return;
    end
    ...
    elseif(strcmp(Fex_Num.Fex,'Perturbation'))
        ...
        convStruct     = DataStorage(['FexData' filesep class(aLine)],@FexMatrices_Meanfield,paramsFex,aLine);
        getConv        = @Fex_Meanfield;
    elseif(strcmp(Fex_Num.Fex,'Zero'))
        getFex         = @zeroFunction;
        IntMatrFex     = [];
        getConv        = @zeroFunction;
        convStruct     = [];
   ...
   %***************************************************************
    ...
    function dxvdt = dxv_dt(t,x)
        ...
        rhoInv   = exp(-(x-Vext)/kBT);
        rhoInv(rhoInv>10^10) = 0;

        dvdt  = dvdt + eta*mInv.*rhoInv.*(Diff.DDy*v);
