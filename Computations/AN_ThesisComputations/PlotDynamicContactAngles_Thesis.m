function PlotDynamicContactAngles_Thesis
   ...
   % PlotThetaOverCa(data);  
    ...
    function PlotFHoffmanVsFGR()        
       ...
       thetaM = HoffmannFit.theta'*pi/180;%(0:0.01:pi)';
        G_thM  = GHR_lambdaEta0(thetaM);    
        for k = 1:length(L_Lambda)
            f_GR = G_thM/log(L_Lambda(k));            
            loglog(f_hoffmann,f_GR,['o',lines{k}]); hold on;
        end
        loglog(f_hoffmann,f_hoffmann,'b-');
        
        xlabel('$F_H$','Interpreter','Latex');
        ylabel('$F_{G}$','Interpreter','Latex');
    end
