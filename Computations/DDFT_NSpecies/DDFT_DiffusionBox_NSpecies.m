function data = DDFT_DiffusionBox_NSpecies(optsPhys,optsNum,optsPlot)
%************************************************************************* 
% data = DDFT_DiffusionBox_NSpecies(optsPhys,optsNum,optsPlot)
%
% define
...
%*************************************************************************    
    if(nargin==0)
        [optsNum,optsPhys] = Test0_DDFT_DiffusionBox_2Species(false);
    end
...
%    getFex = str2func(['Fex_',optsNum.FexNum.Fex]);                
    ...
    tic                  
    abox               = Box(PhysArea);
    [Pts,Diff,Int,Ind] = abox.ComputeAll(optsNum.PlotArea);                            
    convStruct         = FexMatrices_Meanfield(optsPhys,abox);          
    
    abox.doPlots(convStruct(1,1).Conv*ones(size(Pts.y1_kv)),'SC');
    ...
    tic 
     ...
     if(flag<0)
         fprintf(1,'fsolve failed to convergene\n');
         pause
     else
