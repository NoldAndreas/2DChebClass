function output = DDFT_DiffusionPolarInf(optsPhys,optsNum)
%************************************************
...
%************************************************
    ...
    if(nargin == 0)
        [optsNum,optsPhys] = Default1_DDFT_DiffusionPolarInfinity();
    end
    ...
    optsPhys.FexNum           = struct('N',[14,14],'L',L);
    [Pts,Diff,Int,Ind,Interp] = IDC.ComputeAll(optsNum.PlotArea);
    
    opts = optsPhys;    opts.optsNum = optsNum;
    convStruct                = DataStorage(['InfDisc' filesep 'FexMatrices_Meanfield'],@FexMatrices_Meanfield,opts,IDC,false);    
    ...
    function dxdt = dx_dt(t,x)
        ...
        mu_s  = GetChemicalPotential(x,t); %mu_s  = x + Conv*rho_s - mu + getVAdd(Pts.y1_kv,Pts.y2_kv,t,optsPhys.V1);
    ...
    function y = f(x)
        ...
        y            = GetChemicalPotential(x,0); %y            = x + Conv*rho_full - mu + getVAdd(Pts.y1_kv,Pts.y2_kv,0,optsPhys.V1);        
    ...
    function mu_s = GetChemicalPotential(xs,t)
        rho_s = exp((xs-Vext)/kBT);       
        mu_s  = xs + Conv*rho_s - mu + getVAdd(Pts.y1_kv,Pts.y2_kv,t,optsPhys.V1);   
    end
