function [VBack_S,VAdd_S]=V1_Well_gravity_x(y1S,y2S,t,optsPhys)
    ...
    VAdd = VAdd + g*y1S*(1-exp(-t^2)).*exp(-y1S.^2/gcut);
    
    VAdd(abs(y1S)==inf |  abs(y2S)==inf) = 0;
    
    DVAddDy1   = -2*(y1S-y10).*VAdd./sigma1Add;
    DVAddDy2   = -2*(y2S-y20).*VAdd./sigma2Add + exp(-y1S.^2/gcut)*g*(1-exp(-t^2)).*(1-2*y2S.^2/gcut);
